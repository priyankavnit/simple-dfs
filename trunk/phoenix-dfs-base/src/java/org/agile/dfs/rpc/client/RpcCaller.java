...
public class RpcCaller extends RpcCallBase {
    ...
    public RpcCaller(Endpointable endpoint) {
        super(endpoint);
    }

    public RpcResponse call(RpcRequest req) throws IOException {
        this.send(req);
        RpcResponse resp = this.receive();
        if (resp == null) {
            throw new RpcOperateException("Not rpc response! ");
        }
        return resp;
    }
